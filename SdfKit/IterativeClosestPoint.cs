...
public class IterativeClosestPoint
{
    ...
    // public int Corresponses { get; } = 1000;

    public float Tolerance { get; } = 0.01f;

    const float convergedTranslation = 0.001f;
    const float convergedRotation = 0.001f;
    ...
    public Matrix4x4 RegisterPoints (Span<Vector3> points)
    {
        ...
        for (var iter = 0; !converged && iter < MaxIterations; iter++) {
            ...
            converged = dtrans <= convergedTranslation && drot <= convergedRotation;
    ...
    Matrix4x4 GetIterTransform (Span<Vector3> points, Matrix<float> p, Matrix<float> q)
    {
        ...
        float distMax;
        if (distMean < Tolerance) {
            distMax = distMean + 3.0f * distStd;
        }
        else if (distMean < 3.0f * Tolerance) {
            distMax = distMean + 2.0f * distStd;
        }
        else if (distMean < 6.0f * Tolerance) {
        ...
        var translation = qmean - Vector3.Transform (pmean, rMatrix);
